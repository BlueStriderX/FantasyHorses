...
import net.thederpgamerx.fantasyhorses.old.main.PlayerData;
...
public class HorseGUI implements Listener, CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if(sender instanceof Player) {
			...
			if(player.hasPermission("fhorses.gui") || player.hasPermission("fhorses.admin")) {
				openGUI(createHorseSelection(fetchPlayerData(player)), player);
			}
		}
		return true;
	}

	private PlayerData fetchPlayerData(Player player) {
		UUID playerUUID = player.getUniqueId();
		Yaml dataFile = new Yaml(new Constructor(PlayerData.class));
		InputStream inputStream = ;
		return dataFile.load(inputStream);
	}

	private void openGUI(Inventory inventory, Player player) {

	}

	private Inventory createHorseSelection(PlayerData playerData) {
		Inventory horseSelection = Bukkit.createInventory(null, 9, ChatColor.AQUA + playerData.getName() + "'s Horses:");
		int maxHorses = playerData.getHorseSlotsMax();
		int registeredHorses = 0;
		int slotID = 4; //4 is center slot (I think)
		while(registeredHorses <= maxHorses) {
			horseSelection.setItem(slotID, playerData.get);
		}
